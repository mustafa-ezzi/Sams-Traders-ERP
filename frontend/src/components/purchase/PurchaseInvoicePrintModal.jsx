...
const lineItemType = (line) =>
  (line.itemType || line.item_type) === "RAW_MATERIAL" ? "Raw material" : "Finished good";
...
const PurchaseInvoicePrintModal = ({ invoice, onClose, formatDisplayDate }) => {
  ...
  const dimension =
    typeof localStorage !== "undefined" ? localStorage.getItem("tenantId") || "" : "";
    ...
    <>
      ...
      >
        <div className="pi-print-toolbar flex shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-[#0d1424] px-4 py-3 text-white">
          <p id="pi-print-title" className="text-sm font-semibold text-slate-200">
            Print preview · {invNo}
          </p>
          ...
          <article className="pi-print-sheet mx-auto max-w-[210mm] overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl shadow-slate-900/20 print:shadow-none">
            <header className="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-blue-600 to-cyan-600 px-8 py-10 text-white print:bg-indigo-700">
              ...
              <div className="relative flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.35em] text-white/80">Purchase</p>
                  <h1 className="mt-1 text-3xl font-black tracking-tight md:text-4xl">Invoice</h1>
            ...
            <div className="grid gap-6 border-b border-slate-100 bg-slate-50/80 px-8 py-6 md:grid-cols-2">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Warehouse</p>
              ...
              <div className="md:col-span-2 grid gap-4 sm:grid-cols-3">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Printed</p>
                    ...
                    {lines.map((line, index) => {
                      ...
                      return (
                        <tr key={line.id || index} className="text-slate-800">
                          ...
                          <td className="hidden px-3 py-3 text-slate-600 sm:table-cell">{lineItemType(line)}</td>
                          <td className="px-3 py-3 text-right tabular-nums">{formatDecimal(qty)}</td>
                          <td className="hidden px-3 py-3 text-slate-600 md:table-cell">{uom}</td>
                          <td className="px-3 py-3 text-right tabular-nums">{formatDecimal(rate)}</td>
              ...
              <div className="mt-8 flex flex-col items-end gap-2 border-t border-slate-200 pt-6">
                <div className="w-full max-w-xs space-y-2 text-sm">
                  <div className="flex justify-between text-slate-600">
                    ...
                    <span className="font-semibold tabular-nums text-slate-900">{formatDecimal(gross)}</span>
                  </div>
                  <div className="flex justify-between text-slate-600">
                    <span>Invoice discount</span>
                    <span className="font-semibold tabular-nums text-slate-900">− {formatDecimal(invDisc)}</span>
                  </div>
            ...
            <footer className="border-t border-slate-100 bg-slate-50 px-8 py-5 text-center text-xs text-slate-500">
              ...
              <p className="mt-1">This document was generated from CoreLedger and is valid without signature unless otherwise required.</p>
            </footer>
          </article>
        </div>
      </div>
