...
const CreateUpdateSalesReturn = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const linePayload = await salesReturnService.getInvoiceLines(
  ...
  const handleInvoiceChange = async (salesInvoiceId) => {
    setForm((current) => ({ ...current, salesInvoiceId, lines: [] }));
  ...
  const title = editingId ? "Edit Sales Return" : "Sales Return";
  if (loadingRecord) {
    return (
      ...
      <Card className="space-y-6">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">
            ...
            >
              {" "}
              <span>Product</span> <span>Sold Qty</span> <span>Return Qty</span>{" "}
              <span>Unit</span> <span>Rate</span> <span>Amount</span>{" "}
            </div>{" "}
          ...
          <div className="flex justify-end border-t border-slate-200 dark:border-slate-700 pt-4">
            {" "}
            <div className="min-w-[220px] rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-3 text-right">
              {" "}
...
export default CreateUpdateSalesReturn;
